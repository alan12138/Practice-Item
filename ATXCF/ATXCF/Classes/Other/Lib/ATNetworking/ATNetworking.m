                       ...
                       failure:(void (^)(ATURLSessionDataTask *myTask, ATError *myError))failure
{
    ATURLSessionDataTask *dataTask = nil;
     dataTask = [[AFHTTPSessionManager manager] POST:URLString parameters:parameters progress:^(NSProgress * _Nonnull uploadProgress) {
...
{
    ATURLSessionDataTask *dataTask = nil;
    dataTask = [[AFHTTPSessionManager manager] GET:URLString parameters:parameters progress:^(NSProgress * _Nonnull uploadProgress) {
        myProgress(uploadProgress);
    } success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
        success(task,responseObject);
    } failure:^(NSURLSessionDataTask * _Nullable task, NSError * _Nonnull error) {
